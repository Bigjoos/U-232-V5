...
class imdb extends movie_base
{
    ...
    protected function title_year()
    {
        ...
        if (@preg_match('!<title>(IMDb\s*-\s*)?(?<ititle>.*)(\s*-\s*IMDb)?</title>!', $this->page["Title"], $imatch)) {
            ...
            if (preg_match('!class="title-extra" itemprop="name"\s*>\s*"?(.*?)"?\s*<i>!s', $this->page["Title"], $otitle)) {
    ...
    public function get_episode_details()
    {
        ...
        if (preg_match($preg, $this->page["Title"], $match)) {
            $info = ["imdbid"=>$match['seriesimdbid'], "seriestitle"=>$match['seriestitle'], "series_prodtime"=>'', "episodetitle"=>strip_tags($match['episodetitle']),
     ...
     *
     * @return bool success (TRUE if found, FALSE otherwise)
     *
     * @see IMDB page / (TitlePage)
     */
            ...
            if (preg_match('!<select id="bySeason"(.*?)</select!ims', $this->page["Episodes"], $match)) {
                ...
                for ($i=0;$i<count($matches[0]);++$i) {
                    ...
                    if (empty($this->page["Episodes-$s"])) {
                        $this->openpage("Episodes-$s");
                    }
                    if ($this->page["Episodes-$s"] == "cannot open page") {
                        continue;
                    } // no such page
        ...
        if (empty($this->goofs)) {
            ...
            if (@preg_match_all('@<h4 class="li_group">(.+?)(!?&nbsp;)</h4>\s*(.+?)\s*(<h4 class="li_group">|<div id="top_rhs_wrapper")@ims', $this->page["Goofs"], $matches)) {
                ...
                for ($i=0;$i<$gc;++$i) {
                    ...
                    $ic = count($goofy[0]);
                    for ($k=0;$k<$ic;++$k) {
                        $this->goofs[] = ["type"=>$matches[1][$i], "content"=>str_replace('href="/', 'href="http://' . $this->imdbsite . '/', $goofy[1][$k])];
                    }
                }
            }
        }
            ...
            if ($has_trailers !== false) { // if any on-site or off-site trailers exists
                ...
                if (preg_match_all($regex, $html_trailer, $matches)) {
                    ...
                    for ($i=0;$i<count($matches[0]);++$i) {
                        ...
                        if ($full) {
                            $this->trailers[] = ["lang"=>'', "title"=>html_entity_decode($matches[2][$i], ENT_QUOTES, 'UTF-8'), "url"=>$trailer, "restful_url"=>'', "resolution"=>$res];
                        } else {
        ...
        if (empty($this->soundtracks)) {
            ...
            if (preg_match_all('!class="soundTrack soda (odd|even)"\s*>\s*(?<title>.+?)<br\s*/>(?<desc>.+?)</div>!ims', str_replace("\n", " ", $this->page["Soundtrack"]), $matches)) {
                ...
                for ($i=0;$i<$mc;++$i) {
                    ...
                    if (preg_match_all('|^\s*(.*?)\s+by\s+(<a href[^>]+>.+?</a>)|i', $matches['desc'][$i], $match1)) {
                        for ($k=0;$k<count($match1[0]);++$k) {
                            switch ($match1[1][$k]) {
                case "Arranged": $s['credits'][] = ['credit_to'=>str_replace('href="/', 'href="http://' . $this->imdbsite . '/', $match1[2][$k]), 'desc'=>'arrangement']; break;
                case "Composed": $s['credits'][] = ['credit_to'=>str_replace('href="/', 'href="http://' . $this->imdbsite . '/', $match1[2][$k]), 'desc'=>'composer']; break;
                case "Performed": $s['credits'][] = ['credit_to'=>str_replace('href="/', 'href="http://' . $this->imdbsite . '/', $match1[2][$k]), 'desc'=>'performer']; break;
                case "Written": $s['credits'][] = ['credit_to'=>str_replace('href="/', 'href="http://' . $this->imdbsite . '/', $match1[2][$k]), 'desc'=>'writer']; break;
                case "Written and Produced": {
                  $s['credits'][] = ['credit_to'=>str_replace('href="/', 'href="http://' . $this->imdbsite . '/', $match1[2][$k]), 'desc'=>'writer'];
                  $s['credits'][] = ['credit_to'=>str_replace('href="/', 'href="http://' . $this->imdbsite . '/', $match1[2][$k]), 'desc'=>'producer'];
                } break;
                case "Written and Performed": {
                  $s['credits'][] = ['credit_to'=>str_replace('href="/', 'href="http://' . $this->imdbsite . '/', $match1[2][$k]), 'desc'=>'writer'];
                  $s['credits'][] = ['credit_to'=>str_replace('href="/', 'href="http://' . $this->imdbsite . '/', $match1[2][$k]), 'desc'=>'performer'];
                } break;
                default: $s['credits'][] = ['credit_to'=>str_replace('href="/', 'href="http://' . $this->imdbsite . '/', $match1[2][$k]), 'desc'=>'**' . $match1[1][$k] . '**'];
              }
                        }
                ...
                case "Arranged": foreach ($creds as $cred) {
                    $s['credits'][] = ['credit_to'=>str_replace('href="/', 'href="http://' . $this->imdbsite . '/', $cred), 'desc'=>'arrangement'];
                } break;
                case "Composed": foreach ($creds as $cred) {
                    $s['credits'][] = ['credit_to'=>str_replace('href="/', 'href="http://' . $this->imdbsite . '/', $cred), 'desc'=>'composer'];
                } break;
                case "Performed": foreach ($creds as $cred) {
                    $s['credits'][] = ['credit_to'=>str_replace('href="/', 'href="http://' . $this->imdbsite . '/', $cred), 'desc'=>'performer'];
                } break;
                case "Written": foreach ($creds as $cred) {
                    $s['credits'][] = ['credit_to'=>str_replace('href="/', 'href="http://' . $this->imdbsite . '/', $cred), 'desc'=>'writer'];
                } break;
                case "Written and Produced": foreach ($creds as $cred) {
                    $s['credits'][] = ['credit_to'=>str_replace('href="/', 'href="http://' . $this->imdbsite . '/', $cred), 'desc'=>'writer'];
                    $s['credits'][] = ['credit_to'=>str_replace('href="/', 'href="http://' . $this->imdbsite . '/', $cred), 'desc'=>'producer'];
                }
                   break;
                ...
                default: foreach ($creds as $cred) {
                    $s['credits'][] = ['credit_to'=>str_replace('href="/', 'href="http://' . $this->imdbsite . '/', $cred), 'desc'=>'**' . $match1[1][$k] . '**'];
                } break;
              }
                        }
                    }
        ...
        if (empty($this->release_info)) {
            ...
            for ($i=0;$i<$mc;++$i) {
                ...
                if (preg_match('!href="/date/(\d{2})-(\d{2})/">\d+ (.*?)</a>\s*<a href="/year/(\d{4})/">!is', $matches[2][$i], $match)) { // full info
                    $this->release_info[] = ['country'=>$country, 'day'=>$match[2], 'month'=>$match[3], 'mon'=>$match[1], 'year'=>$match[4], 'comment'=>$matches[3][$i]];
                } elseif (preg_match('!(\d{1,2})\s*(.+?)<a href="/year/(\d{4})/.+?"\s*>!is', $matches[2][$i], $match)) { // full info v2
                    $this->release_info[] = ['country'=>$country, 'day'=>$match[1], 'month'=>trim($match[2]), 'mon'=>$this->monthNo(trim($match[2])), 'year'=>$match[3], 'comment'=>$matches[3][$i]];
                } elseif (!preg_match('|a href=|i', $matches[2][$i], $match)) { // no links within
          if (preg_match('!^(.+?)\s(\d{4})$!s', trim($matches[2][$i]), $match)) { // month and year
            $this->release_info[] = ['country'=>$country, 'day'=>'', 'month'=>$match[1], 'mon'=>$this->monthNo(trim($match[1])), 'year'=>$match[2], 'comment'=>$matches[3][$i]];
          } elseif (preg_match('!(\d{4})!', trim($matches[2][$i]), $match)) { // year at least
              $this->release_info[] = ['country'=>$country, 'day'=>'', 'month'=>'', 'mon'=>'', 'year'=>$match[1], 'comment'=>$matches[3][$i]];
          }
                } else {
    ...
    public function locations()
    {
        if (empty($this->locations)) {
            ...
            if (preg_match_all('!<dt>(<a href="(.*?)">|)(.*?)(</a>|</dt>)!ims', $block, $matches)) {
                for ($i=0;$i<count($matches[0]);++$i) {
                    $this->locations[] = ['name'=>$matches[3][$i], 'url'=>$matches[2][$i]];
                }
            }
        }
    ...
    protected function companyParse(&$text, &$target)
    {
        ...
        $mc = count($matches[0]);
        for ($i=0;$i<$mc;++$i) {
            $target[] = ["name"=>$matches[2][$i], "url"=>'http://' . $this->imdbsite . $matches[1][$i], "notes"=>trim($matches[3][$i])];
        }
    }
