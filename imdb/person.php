<?php
 ...
 /* $Id: person.php 634 2013-12-01 20:02:41Z izzy $ */
...
if (isset($_GET["mid"]) && preg_match('/^[0-9]+$/', $_GET["mid"])) {
    ...
    if (!empty($birthday)) {
        ...
        if (!empty($birthday["place"])) {
            echo "<br>in " . $birthday["place"];
        }
        echo "</div>";
    }
    ...
    if (!empty($death)) {
        ...
        if (!empty($death["place"])) {
            echo "<br>in " . $death["place"];
        }
        if (!empty($death["cause"])) {
            echo "<br>Cause: " . $death["cause"];
        }
        echo "</div>";
    }
    ...
    if (!empty($sp)) {
        ...
        foreach ($sp as $spouse) {
            ...
            if (empty($spouse["from"])) {
                ...
                if (!empty($spouse["to"])) {
                    echo " - " . $spouse["to"]["day"] . "." . $spouse["to"]["month"] . " " . $spouse["to"]["year"];
                }
                echo "</td>";
            }
            if (empty($spouse["comment"])&&empty($spouse["children"])) {
                ...
                echo "<td>";
                if (empty($spouse["comment"])&&!empty($spouse["children"])) {
                    echo "Kids: " . $spouse["children"] . "</td></tr>";
                } elseif (empty($spouse["children"])&&!empty($spouse["comment"])) {
                    echo $spouse["comment"] . "</td></tr>";
                } else {
                    echo $spouse["comment"] . "; Kids: " . $spouse["children"] . "</td></tr>";
                }
            }
        }
        echo "</table></TD></TR>\n";
    ...
    if (!empty($sal)) {
        ...
        for ($i=0;$i<$tc;++$i) {
            echo "<tr><td>";
            if (!empty($sal[$i]["movie"]["imdb"])) {
                echo "<a href='movie.php?mid=" . $sal[$i]["movie"]["imdb"] . "&engine=" . $engine . "'>" . $sal[$i]["movie"]["name"] . "</a>";
            } else {
                echo $sal[$i]["movie"]["name"];
            }
            if (!empty($sal[$i]["movie"]["year"])) {
                echo " (" . $sal[$i]["movie"]["year"] . ")";
            }
            ...
            foreach ($filmo as $film) {
                ...
                if (empty($film["chname"])) {
                    ...
                    if (empty($film["chid"])) {
                        ...
                        echo "<a href='http://" . $person->imdbsite . "/character/ch" . $film["chid"] . "/'>" . $film["chname"] . "</a>";
                    }
                }
                echo "</td></tr>";
            }
    ...
    if (!empty($books)) {
        ...
        for ($i=0;$i<$tc;++$i) {
            ...
            if (!empty($books[$i]["url"])) {
                echo "<a href='" . $books[$i]["url"] . "'>" . $books[$i]["isbn"] . "</a></td></tr>";
        ...
        for ($i=0;$i<$tc;++$i) {
            ...
            if (empty($iv[$i]["author"])) {
                ...
                if (empty($iv[$i]['auturl'])) {
                    ...
                    echo "<a href='http://" . $person->imdbsite . $iv[$i]["auturl"] . "'>" . $iv[$i]["author"] . "</a>";
                }
                echo "</td></tr>";
            }
        }
