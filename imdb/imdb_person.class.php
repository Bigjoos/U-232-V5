    ...
    default:
     ...
     protected function filmograf(&$res, $type)
     {
         ...
         if (!empty($match) && preg_match_all('!<div class="filmo-row.*?>\s*(.*?)\s*</div!ims', $match[1], $matches)) {
            ...
            case 'producer': $chname = 'Producer'; break;
                 ...
                 $res[] = ["mid"=>$mov[1], "name"=>$mov[2], "year"=>$year, "chid"=>$chid, "chname"=>$chname, "addons"=>trim($mov[3])];
             }
         }
     }
     ...
     public function died()
     {
         if (empty($this->deathday)) {
             ...
             if (preg_match('|Date of Death</td>(.*?)</td|ims', $this->page["Bio"], $match)) {
                 ...
                 $this->deathday = ["day"=>@$daymon[2], "month"=>@$daymon[3], "mon"=>@$daymon[1], "year"=>@$dyear[1], "place"=>@trim(strip_tags($dloc[3])), "cause"=>@$dcause[1]];
             }
         }
             ...
             if (preg_match('!<h4 class="li_group">Mini Bio[^>]+?>(.+?)<(h4 class="li_group"|div class="article")!ims', $this->page["Bio"], $block)) {
                 ...
                 for ($i=0;$i<count($matches[0]);++$i) {
                     $bio_bio["desc"] = str_replace(
                         "href=\"/name/nm",
                         "href=\"http://" . $this->imdbsite . "/name/nm",
                         str_replace(
                             "href=\"/title/tt",
                             "href=\"http://" . $this->imdbsite . "/title/tt",
     ...
     public function pubprints()
     {
         if (empty($this->pub_prints)) {
             if (empty($this->page["Publicity"])) {
                 $this->openpage("Publicity", "person");
             }
             ...
             for ($i=1;$i<$pc;++$i) {
                 if (preg_match('/(.*).\s*<i>(.*)<\/i>\s*((.*):|)((.*),|)\s*((\d+)\.|)\s*ISBN\s*<a href="(.*)">(.*)<\/a>/iU', $arr[$i], $match)) {
                     $this->pub_prints[] = ["author"=>$match[1], "title"=>$match[2], "place"=>trim($match[4]), "publisher"=>trim($match[6]), "year"=>$match[8], "isbn"=>$match[10], "url"=>$match[9]];
                 } elseif (preg_match('/(.*).\s*<i>(.*)<\/i>\s*((.*):|)((.*),|)\s*((\d+)\.)/iU', $arr[$i], $match)) {
                     $this->pub_prints[] = ["author"=>$match[1], "title"=>$match[2], "place"=>trim($match[4]), "publisher"=>trim($match[6]), "year"=>$match[8], "isbn"=>"", "url"=>""];
                 }
             }
         }
     ...
     protected function parsearticles(&$res, $page, $title)
     {
         ...
         for ($i=0;$i<$lc;++$i) {
             if (@preg_match('|<td.*?>(.*?)</td><td.*?>(.*?)</td>|ms', $matches[1][$i], $match)) {
                 ...
                 $datum = ["day"=>$dat[1], "month"=>trim($dat[2]), "mon"=>$this->monthNo(trim($dat[2])), "year"=>trim($dat[3]), "full"=>$dat[0]];
                 if (strlen($dat[0])) {
                 ...
                 $res[] = ["inturl"=>'', "name"=>$match[1], "date"=>$datum, "details"=>trim($match[2]), "auturl"=>'', "author"=>$resauthor];
             }
         }
     }
