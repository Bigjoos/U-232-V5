...
class page_verify
{
    ...
    function check($task_name = 'Default')
    {
		...
		$session_task_id = isset($CURUSER['id']) ? $CURUSER['id'] : $INSTALLER09['bot_id'];
        //if (isset($_SESSION['Task']) && $_SESSION['Task'] != md5('user_id:'.$session_task_id.'::taskname-'.$task_name.'::'.$_SESSION['Task_Time'])) stderr("Error", "Please resubmit the form. <a href='".$returl."'>Click HERE</a>", false);
        $this->create();
    }
}
?>
