...
if (mysqli_num_rows($res) == 0) {
    ...
    while ($arr = mysqli_fetch_assoc($res)) {
        $color = '#333333';
        if (strpos($arr['txt'], $lang['log_uploaded'])) {
            $color = "#4799ad";
        }
        if (strpos($arr['txt'], $lang['log_created'])) {
            $color = "#CC9966";
        }
        if (strpos($arr['txt'], $lang['log_section'])) {
            $color = "#ba79d8";
        }
        if (strpos($arr['txt'], $lang['log_started'])) {
            $color = "#00E300";
        }
        if (strpos($arr['txt'], $lang['log_finished'])) {
            $color = "#00E300";
        }
        if (strpos($arr['txt'], $lang['log_sticky'])) {
            $color = "#BBaF9B";
        }
        if (strpos($arr['txt'], $lang['log_invited_by'])) {
            $color = "#CC9966";
        }
        if (strpos($arr['txt'], $lang['log_invited_to'])) {
            $color = "#CC9966";
        }
        if (strpos($arr['txt'], $lang['log_deleted_by'])) {
            $color = "#CC6666";
        }
        if (strpos($arr['txt'], $lang['log_deleted_system'])) {
            $color = "#FF6600";
        }
        if (strpos($arr['txt'], $lang['log_sent'])) {
            $color = "#af0b0b";
        }
        if (strpos($arr['txt'], $lang['log_reason'])) {
            $color = "#d34e29";
        }
        if (strpos($arr['txt'], $lang['log_user'])) {
            $color = "#d34e29";
        }
        if (strpos($arr['txt'], $lang['log_promoted'])) {
            $color = "#3ae2f1";
        }
        if (strpos($arr['txt'], $lang['log_demoted'])) {
            $color = "#375d60";
        }
        if (strpos($arr['txt'], $lang['log_updated'])) {
            $color = "#6699FF";
        }
        if (strpos($arr['txt'], $lang['log_edited'])) {
            $color = "#BBaF9B";
        }
