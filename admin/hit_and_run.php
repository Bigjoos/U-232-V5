...
$query = (isset($_GET['really_bad']) ? 'SELECT COUNT(*) FROM xbt_peers LEFT JOIN users ON users.id = xbt_peers.uid WHERE xbt_peers.completed >= \'1\' AND xbt_peers.hit_and_run > 0 AND users.hit_and_run_total > 2' : 'SELECT COUNT(*) FROM `xbt_peers` WHERE `completed` >= \'1\' AND `hit_and_run` > 0');
}
$HTMLOUT = '';
...
$query_2 = (isset($_GET['really_bad']) ? 'SELECT x.tid, x.uid, x.hit_and_run, x.downloaded AS dload, x.uploaded AS uload, x.seedtime, x.started, x.completedtime, x.active, u.id, u.avatar, u.username, u.uploaded AS up, u.downloaded AS down, u.class, u.hit_and_run_total, u.donor, u.warned, u.enabled, u.leechwarn, u.chatpost, u.pirate, u.king, u.suspended, t.owner, t.name, t.added AS torrent_added, t.seeders AS numseeding, t.leechers AS numleeching FROM xbt_peers AS x LEFT JOIN users AS u ON u.id = x.uid LEFT JOIN torrents AS t ON t.id=x.tid WHERE completed >= \'1\' AND hit_and_run > 0 AND u.hit_and_run_total > 2 ORDER BY uid ' . $LIMIT : 'SELECT x.tid, x.uid, x.hit_and_run, x.downloaded AS dload, x.uploaded AS uload, x.seedtime, x.started, x.completedtime, x.active, u.id, u.avatar, u.username, u.uploaded AS up, u.downloaded AS down, u.class, u.hit_and_run_total, u.donor, u.warned, u.enabled, u.leechwarn, u.chatpost, u.pirate, u.king, u.suspended, t.owner, t.name, t.added AS torrent_added, t.seeders AS numseeding, t.leechers AS numleeching FROM xbt_peers AS x LEFT JOIN users AS u ON u.id = x.uid LEFT JOIN torrents AS t ON t.id=x.tid WHERE `completed` >= \'1\' AND `hit_and_run` > 0 ORDER BY `uid` ' . $LIMIT);
}
...
while ($hit_and_run_arr = mysqli_fetch_assoc($hit_and_run_rez)) {
    //=== Xbt Tracker or Default Announce
        ...
        $T_ID = (XBT_TRACKER === true ? $hit_and_run_arr['tid'] : $hit_and_run_arr['torrentid']);
