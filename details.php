...
if (($torrents = $cache->get('torrent_details_' . $id)) === false) {
   ...
   if ($change[$torrents['category']]['min_class'] > $CURUSER['class']) {
       stderr("{$lang['details_user_error']}", "{$lang['details_bad_id']}");
   }
//==
...
<ul class='nav nav-tabs'>
  ...
  <li><a href='#tab_c' data-toggle='tab'>{$lang['details_add_men3']}</a></li>";
    if ($CURUSER['class'] >= UC_POWER_USER) {
        $HTMLOUT .= "<li><a href='#tab_d' data-toggle='tab'>{$lang['details_add_men4']}</a></li>";
    }
...
if ($torrents["nuked"] == "yes") {
    $HTMLOUT.= "<tr><td class='rowhead'><b>{$lang['details_add_nuk1']}</b></td><td align='left'><img src='{$INSTALLER09['pic_base_url']}nuked.gif' alt='Nuked' title='Nuked' /></td></tr>\n";
}
...
<h3 class='text-center'>{$lang['details_add_snatch1']}<a href='{$INSTALLER09['baseurl']}/details.php?id=" . (int) $torrents['id'] . "'>" . htmlsafechars($torrents['name']) . "</a><br />{$lang['details_add_snatch2']}{$Row_Count['0']}{$lang['details_add_snatch3']}" . ($Row_Count[0] == 1 ? "" : "es") . "</h3>\n";
    ...
    if (($Detail_Snatch = $cache->get($What_cache . $id)) === false) {
        ...
        while ($snatched_torrent = mysqli_fetch_assoc($Main_Q)) {
...
if (empty($tvmaze_info) && empty($imdb) && in_array($torrents['category'], array_merge($INSTALLER09['movie_cats'], $INSTALLER09['tv_cats']))) {
    $HTMLOUT.= "<tr><td colspan='2'>{$lang['details_add_noimdb']}</td></tr>";
}
$HTMLOUT.= "</table>
