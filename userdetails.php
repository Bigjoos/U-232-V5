...
if ((($user['class'] == UC_MAX or $user['id'] == $CURUSER['id']) || ($user['class'] < UC_MAX) && $CURUSER['class'] == UC_MAX) && isset($_GET['invincible'])) {
    ...
    if ($_GET['invincible'] == 'yes') {
        $HTMLOUT.= "<div class='row'><div class'col-md-4'>" . invincible($id) . "</div></div";
    } elseif ($_GET['invincible'] == 'remove_bypass') {
        $HTMLOUT.= "<div class='row'><div class'col-md-4'>" . invincible($id, true, false);
    } else {
...
if ((($user['class'] >= UC_STAFF or $user['id'] == $CURUSER['id']) || ($user['class'] < UC_STAFF) && $CURUSER['class'] >= UC_STAFF) && isset($_GET['stealth'])) {
    ...
    if ($_GET['stealth'] == 'yes') {
        $HTMLOUT.= "<div class='row'><div class'col-md-4'>" . stealth($id) . "</div></div";
    } elseif ($_GET['stealth'] == 'no') {
        $HTMLOUT.= "<div class='row'><div class'col-md-4'>" . stealth($id, false) . "</div></div";
    }
} // End
//==country by pdq
function countries()
...
if ($CURUSER['id'] != $user['id']) {
    $HTMLOUT.= "<div class='col-sm-2'><h4><a href='{$INSTALLER09['baseurl']}/sharemarks.php?id=$id'>{$lang['userdetails_sharemarks']}</a></h4></div>";
}
...
if (!$enabled) {
    ...
    if ($friends > 0) {
        ...
        $HTMLOUT.= "<div class='row'><div class='col-md-1'><p><a href='friends.php?action=add&amp;type=friend&amp;targetid=$id'>{$lang['userdetails_add_friends']}</a></p></div>";
    }
    if ($blocks > 0) {
        $HTMLOUT.= "<div class='col-md-1'><p><a href='friends.php?action=delete&amp;type=block&amp;targetid=$id'>{$lang['userdetails_remove_blocks']}</a></p></div>\n";
    } else {
        $HTMLOUT.= "<div class='row'><div class='col-md-1'><p><a href='friends.php?action=add&amp;type=block&amp;targetid=$id'>{$lang['userdetails_add_blocks']}</a></p></div>\n";
    }
}
...
if (!in_array($action, $possible_actions)) {
    stderr('Error', '<br><div class="alert alert-error span11">Error! Change a few things up and try submitting again.</div>');
}
             $HTMLOUT.= "</div>
<div class='container'>
...
if ($action == "general") {
    ...
    if (($CURUSER['id'] !== $user['id']) && ($CURUSER['class'] >= UC_STAFF)) {
        ...
        if (!empty($user_stats['modcomment'])) {
            $HTMLOUT.= "<tr><td class='rowhead'>{$lang['userdetails_system']}</td><td align='left'>" . ($user_stats['modcomment'] != '' ? $the_flip_box_7 . '<div align="left" id="box_7" style="display:none"><hr>' . format_comment($user_stats['modcomment']) . '</div>' : '') . "</td></tr>";
        }
    }
    //==Begin blocks
...
if ($action == "edit") {
    ...
    <div class='col-sm-4'><input class='form-control' placeholder='{$lang['userdetails_website']}' type='text' name='website' value='" . htmlsafechars($user['website']) . "'></div></div><br>";
        ...
        if ($CURUSER['class'] == UC_STAFF && $user["class"] > UC_VIP) {
            $HTMLOUT.= "<input class='form-control'type='hidden' name='class' value='{$user['class']}'>";
        } else {
        ...
        if ($CURUSER['class'] >= UC_ADMINISTRATOR && XBT_TRACKER == false) {
            ...
            if ($free_switch) {
                if ($user['free_switch'] == 1) {
                    $HTMLOUT.= '<div class="col-sm-2">(' . $lang['userdetails_unlimited_d'] . ')</div>';
                } else {
                    $HTMLOUT.= "<div class='col-sm-2'>{$lang['userdetails_until']} " . get_date($user['free_switch'], 'DATE') . " (" . mkprettytime($user['free_switch'] - TIME_NOW) . " {$lang['userdetails_togo']})</div>";
                }
            } else {
        ...
        if ($CURUSER['class'] >= UC_STAFF && XBT_TRACKER == false) {
            ...
            if ($downloadpos) {
                if ($user['downloadpos'] == 0) {
                    $HTMLOUT.= '<div class="col-sm-1">(' . $lang['userdetails_unlimited_d'] . ')</div>';
                } else {
                    $HTMLOUT.= "<div class='col-sm-2'>{$lang['userdetails_until']} " . get_date($user['downloadpos'], 'DATE') . " (" . mkprettytime($user['downloadpos'] - TIME_NOW) . " {$lang['userdetails_togo']})</div><div class='col-sm-6'><!--<input class='form-control' placeholder='Comments' type='text' name='disable_pm'></div></div>";
                }
            } else {
        ...
        if ($CURUSER['class'] >= UC_STAFF) {
            ...
            if ($uploadpos) {
                if ($user['uploadpos'] == 0) {
                    ...
                    $HTMLOUT.= "<div class='col-sm-2'>{$lang['userdetails_until']} " . get_date($user['uploadpos'], 'DATE') . " (" . mkprettytime($user['uploadpos'] - TIME_NOW) . " {$lang['userdetails_togo']})</div><div class='col-sm-6'><!--<input class='form-control' placeholder='Comments' type='text' name='updisable_pm'>--></div></div>";
                }
            } else {
        ...
        if ($CURUSER['class'] >= UC_STAFF) {
            ...
            if ($sendpmpos) {
                if ($user['sendpmpos'] == 0) {
                    ...
                    $HTMLOUT.= "<div class='col-sm-2'>{$lang['userdetails_until']} " . get_date($user['sendpmpos'], 'DATE') . " (" . mkprettytime($user['sendpmpos'] - TIME_NOW) . " {$lang['userdetails_togo']})</div><div class='col-sm-6'><!--<input placeholder='Comments' class='form-control' type='text' name='pmdisable_pm'>--></div></div>";
                }
            } else {
        ...
        if ($CURUSER['class'] >= UC_STAFF) {
            ...
            if ($chatpost) {
                if ($user['chatpost'] == 0) {
                    ...
                    $HTMLOUT.= "<div class='col-sm-2'>{$lang['userdetails_until']} " . get_date($user['chatpost'], 'DATE') . " (" . mkprettytime($user['chatpost'] - TIME_NOW) . " {$lang['userdetails_togo']})</div><div class='col-sm-6'><!--<input placeholder='Comments' class='form-control' type='text' name='chatdisable_pm'>--></div></div>";
                }
            } else {
          ...
          <div class='col-sm-2'>" . ($avatarpos ? "<input name='avatarpos' value='42' type='radio'>{$lang['userdetails_remove_avatar_d']}" : $lang['userdetails_no_disablement']) . "</div>";
            if ($avatarpos) {
                if ($user['avatarpos'] == 0) {
                    ...
                    $HTMLOUT.= "<div class='col-sm-2'>{$lang['userdetails_until']} " . get_date($user['avatarpos'], 'DATE') . " (" . mkprettytime($user['avatarpos'] - TIME_NOW) . " {$lang['userdetails_togo']})</div><div class='col-sm-6'><!--<input placeholder='Comments' class='form-control' type='text' name='avatardisable_pm'>--></div></div>";
                }
            } else {
        ...
        if ($CURUSER['class'] >= UC_STAFF) {
            ...
            if ($immunity) {
                if ($user['immunity'] == 1) {
                    ...
                    $HTMLOUT.= "<div class='col-sm-2'>{$lang['userdetails_until']} " . get_date($user['immunity'], 'DATE') . " (" . mkprettytime($user['immunity'] - TIME_NOW) . " {$lang['userdetails_togo']})</div><div class='col-sm-6'><!--<input placeholder='Comments' class='form-control' type='text'  name='immunity_pm'>--></div></div>";
                }
            } else {
        ...
        if ($CURUSER['class'] >= UC_STAFF) {
            ...
            if ($leechwarn) {
                if ($user['leechwarn'] == 1) {
                    ...
                    $HTMLOUT.= "<div class='col-sm-2'>{$lang['userdetails_until']} " . get_date($user['leechwarn'], 'DATE') . " (" . mkprettytime($user['leechwarn'] - TIME_NOW) . " {$lang['userdetails_togo']})</div><div class='col-sm-6'><!--<input placeholder='Comments' class='form-control' type='text' size='60' name='leechwarn_pm'>--></div></div>";
                }
            } else {
        ...
        if ($CURUSER['class'] >= UC_STAFF) {
            ...
            if ($warned) {
                if ($user['warned'] == 1) {
                    ...
                    $HTMLOUT.= "<div class='col-sm-2'>{$lang['userdetails_until']} " . get_date($user['warned'], 'DATE') . " (" . mkprettytime($user['warned'] - TIME_NOW) . " {$lang['userdetails_togo']})</div><div class='col-sm-6'><!--<input placeholder='Comments' class='form-control' type='text' name='warned_pm'>--></div></div>";
                }
            } else {
...
<div class='col-sm-2'><input class='form-control' type='text' name='amountdown'></div>
        ...
        if (XBT_TRACKER == true) {
            ...
            // == Peers limit
            if ($CURUSER['class'] >= UC_STAFF) {
                $HTMLOUT.= "<div class='col-sm-1'>{$lang['userdetails_peerslimit']}<input class='form-control' type='text'' name='peers_limit' value='" . (int) $user['peers_limit'] . "'></div>";
            }
            // ==end
            // == Torrents limit
            if ($CURUSER['class'] >= UC_STAFF) {
                $HTMLOUT.= "<div class='col-sm-1'>{$lang['userdetails_torrentslimit']}<input class='form-control' type='text' name='torrents_limit' value='" . (int) $user['torrents_limit'] . "'></div><!--</div>-->";
            }
            // ==end
        }
        ...
        // == seedbonus
        if ($CURUSER['class'] >= UC_STAFF) {
            $HTMLOUT.= "<div class='col-sm-2'>{$lang['userdetails_bonus_points']}<input class='form-control' type='text' name='seedbonus' value='" . (int) $user_stats['seedbonus'] . "'></div>";
        }
        // ==end
        // == rep
        if ($CURUSER['class'] >= UC_STAFF) {
            $HTMLOUT.= "<div class='col-sm-2'>{$lang['userdetails_rep_points']}<input class='form-control' type='text' name='reputation' value='" . (int) $user['reputation'] . "'></div></div>";
        }
        // ==end
