...
if ($INSTALLER09['hnr_online'] == 1 && $user['paranoia'] < 2 || $CURUSER['id'] == $id || $CURUSER['class'] >= UC_POWER_USER) {
    ...
    $r = sql_query("SELECT torrents.name, torrents.added AS torrent_added, xbt_peers.started AS st, xbt_peers.completedtime AS c, xbt_peers.downspeed, xbt_peers.seedtime, xbt_peers.active, xbt_peers.left, xbt_peers.tid as tid, categories.id as category, categories.image, categories.name as catname, xbt_peers.uploaded, xbt_peers.downloaded, xbt_peers.hit_and_run, xbt_peers.mark_of_cain, xbt_peers.completedtime, xbt_peers.mtime, xbt_peers.uid, torrents.seeders, torrents.leechers, torrents.owner FROM xbt_peers JOIN torrents ON torrents.id = xbt_peers.tid JOIN categories ON categories.id = torrents.category WHERE xbt_peers.completed>='1' AND uid=" . sqlesc($id) . " AND torrents.owner != " . sqlesc($id) . " ORDER BY xbt_peers.tid DESC") or sqlerr(__FILE__, __LINE__);
}
    //=== completed
