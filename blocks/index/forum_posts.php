...
$HTMLOUT.= "";
	...
	<div class='panel-body'>
                ...
                if ($topics) {
                    foreach ($topics as $topicarr) {
                        ...
                        if ($topicarr['pan'] == 'yes') {
                            if ($CURUSER['class'] < UC_STAFF && $topicarr['user_id'] != $CURUSER['id']) {
                                ...
                                $username = (!empty($topicarr['username']) ? "<i>{$lang['index_fposts_anonymous']}</i>&nbsp;&nbsp;<a href='" . $INSTALLER09['baseurl'] . "/userdetails.php?id=" . (int) $topicarr['puser_id'] . "'><b>[" . htmlsafechars($topicarr['username']) . "]</b></a>" : "<i>{$lang['index_fposts_unknow']}[$topic_userid]</i>");
                            }
                        } else {
                        ...
                        if ($topicarr['tan'] == 'yes') {
                            if ($CURUSER['class'] < UC_STAFF && $topicarr['user_id'] != $CURUSER['id']) {
                                ...
                                $author = (!empty($topicarr['u2_username']) ? "<i>{$lang['index_fposts_anonymous']}</i>&nbsp;&nbsp;<a href='" . $INSTALLER09['baseurl'] . "/userdetails.php?id=" . $topic_userid . "'><b>[" . htmlsafechars($topicarr['u2_username']) . "]</b></a>" : ($topic_userid == '0' ? "<i>System</i>" : "<i>{$lang['index_fposts_unknow']}[$topic_userid]</i>"));
                            }
                        } else {
                    ...
                    if (empty($topics)) {
                        $HTMLOUT.= "<tr><td class='fullwidth'>{$lang['latestposts_no_posts']}</td></tr></table></div>";
                    }
                }
            }
