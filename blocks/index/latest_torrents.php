...
if (($top5torrents = $cache->get('top5_tor_')) === false) {
    ...
    $cache->set('top5_tor_', $top5torrents, $INSTALLER09['expires']['top5_torrents']);
}
...
<td class='text-center'><span class='badge'>$thealth</td>             
        ...
        if (empty($last5torrents)) {
            $HTMLOUT.= "<tbody><tr><td class='text-left' colspan='3'>{$lang['last5torrents_no_torrents']}</td></tr></tbody></table>";
        }
    }
}
$HTMLOUT.="</div></div></div>";
